...
onMounted(async () => {
  try {
    ...
    const baseNtosResponse = await gqlClient.query({
      ...
      variables: {
        parent_label: 'Tree',
        tree_hash: base_commit.value.fs_root_hash,
        path: NTOSKRNL_PATH
      }
    })
    const diffeeNtosResponse = await gqlClient.query({
      ...
      variables: {
        parent_label: 'Tree',
        tree_hash: diffee_commit.value.fs_root_hash,
        path: NTOSKRNL_PATH
      }
    })
...
<template>
  <div class="container">
    ...
    <BTabs content-class="mt-3" v-if="base_commit.ntos_hash && diffee_commit.ntos_hash">
      <BTab title="Symbols">
        <SymbolDiffView
          :base_blob_hash="base_commit.ntos_hash"
          :diffee_blob_hash="diffee_commit.ntos_hash"
        />
      </BTab>
    </BTabs>
  </div>
